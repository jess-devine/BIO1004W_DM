...
```{r ip_truncated_Poisson}
# Define formulas
formulas <- list(
  incubation_period ~ s(Rainfallspecific),
  incubation_period ~ s(meanmaxTspecific),
  incubation_period ~ te(Groupsize),
  incubation_period ~ s(Rainfallspecific) + te(Groupsize),
  incubation_period ~ s(meanmaxTspecific) + te(Groupsize),
  incubation_period ~ s(meanmaxTspecific) + s(Rainfallspecific),
  incubation_period ~ s(meanmaxTspecific) + s(Rainfallspecific) + te(Groupsize),
  incubation_period ~ s(meanmaxTspecific) + te(Rainfallspecific, Groupsize),
  incubation_period ~ te(meanmaxTspecific, Groupsize) + s(Rainfallspecific),
  incubation_period ~ te(meanmaxTspecific, Rainfallspecific) + te(Groupsize),
  incubation_period ~ te(Rainfallspecific, meanmaxTspecific, Groupsize)
)

# Fit truncated Poisson models
models <- map(formulas, ~ gam(.x, family = quasipoisson(), data = tidy_data))


# Store results in a tibble
model_results <- tibble(
  Model_Name = paste0("incubation_model_", 1:length(formulas)),
  Formula = formulas,
  Model = models,
...
# Best model
best_model <- model_results %>% slice(1)
cat("Best model based on AIC:", best_model$Model_Name, "with AIC =", best_model$AIC, "\n")

# Overdispersion check
residual_deviance <- sum(residuals(best_model$Model, type = "deviance")^2)
dispersion <- residual_deviance / df.residual(best_model$Model)
cat("Truncated Poisson dispersion:", dispersion, "\n")
```